...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['beli'])) {
    ...
    // Ambil data produk
    $query = "SELECT * FROM tb_spotify WHERE id = $id_produk";
    ...
    if ($produk && $produk['stok_barang'] >= $quantity) {
        ...
        $update_query = "UPDATE tb_spotify SET stok_barang = $stok_baru WHERE id = $id_produk";
...
<body class="bg-gray-900 text-white">
    ...
    <div class="container mx-auto p-8">
        <h1 class="text-3xl font-bold text-left mb-8 text-blue-400">Spotify</h1>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            <?php
            // Ambil data produk
            $query = "SELECT * FROM tb_spotify";
            $result = $conn->query($query);

            // Tampilkan produk
            while ($row = $result->fetch_assoc()) {
                // Path gambar berdasarkan folder dan ID produk
                $image_path = "../img/spotify.png"; // Ubah ekstensi jika berbeda (misalnya .png)
